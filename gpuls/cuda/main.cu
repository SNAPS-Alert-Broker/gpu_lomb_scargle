...
/// @brief Runs GPULS on a batch of objects
/// removes all the input/output
...
#pragma omp parallel for num_threads(NSTREAMS) reduction(+ \
														 : totalelemstransfered)
...
void updateYerrorfactor(DTYPE *y, DTYPE *dy, const unsigned int sizeData)
{
	...
	DTYPE sumw = 0;
#pragma omp parallel for num_threads(NTHREADSCPU) reduction(+ \
															: sumw)
